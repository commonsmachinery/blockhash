                ...
                for(i = next_hash_frame; i < HASH_PART_COUNT; ++i) {
                    if(hash_frames[i].frame_number == current_frame) {
                        if(!bmp) {
                            ...
                            // Extract pixels
#if defined(AAA) // Gives flipped image...
                            {
                                int y = 0;
...
#elif defined(BBB)
                            {
                                int _w = state.video_codec_ctx->width * 3;
                                size_t frame_line_size = state.picture.linesize[0];
                                ...
                                while(y < height)
                                {
                                    register uint8_t* src = picture_data;
                                    register uint8_t* dest_end = bmp_data;
                                    register uint8_t* dest = dest_end + _w;
                                    while(dest >= dest_end)
                                        *--dest = *src++;
                                    bmp_data -= image_line_size_with_padding;
                                    picture_data += frame_line_size;
